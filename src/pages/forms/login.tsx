...
/**
 * Télécharge un fichier binaire (Blob) de manière sécurisée via un Bearer Token.
 */
const triggerDownloadWithAxios = async (downloadUrl: string, token: string) => {
  try {
    // 1. Récupération de l'image sous forme de Blob
    ...
    // 2. Création d'un objet URL local
    const blob = new Blob([response.data], { type: response.headers['content-type'] })
    const localUrl = window.URL.createObjectURL(blob)

    // 3. Extraction du nom du fichier
    const fileName = downloadUrl.split('/').pop() || 'photo.jpg'
...
export default function LoginPage() {
  ...
  // États pour la gestion du chargement et des erreurs HTTP
  ...
  try {
    ...
    const response = await axios.post('http://127.0.0.1:8000/api/login-site', {
      email,
      password,
      remember: rememberMe,
    })
    ...
    // 3. Téléchargement séparé (ne doit pas bloquer la connexion s'il échoue)
    const downloadUrl = searchParams.get('downloadUrl')
    if (downloadUrl) {
      try {
        const link = document.createElement('a')
        link.href = downloadUrl
        link.setAttribute('download', downloadUrl.split('/').pop() || 'photo.jpg')
        document.body.appendChild(link)
        link.click()
        link.remove()
      } catch (dlErr) {
        console.error('Erreur téléchargement :', dlErr)
      }
    }

    // 4. Redirection
    const redirectPath = searchParams.get('redirect') || '/galerie'
    ...
    if (err.response?.status === 422) {
      // Erreur de saisie utilisateur (E-mail / Mot de passe)
      ...
      setError('Erreur interne du serveur Laravel (500). Vérifie storage/logs/laravel.log.')
    } else {
      // Pas de réponse (CORS / Serveur crashé / Pas d'internet)
      setError('Impossible de contacter le serveur backend. Vérifie la console F12.')
    }
  } finally {
    // 💡 Débloque le bouton dans TOUS les cas !
    setLoading(false)
  }
}
  ...
  return (
    <div className="min-h-screen bg-[#F1F5F9] flex items-center justify-center p-4 md:p-10 font-['Poppins']">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-6xl overflow-hidden grid grid-cols-1 lg:grid-cols-12 min-h-[750px]">
          ...
          <div className="max-w-md w-full mx-auto my-auto py-8">
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-5">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-blue-600 text-white py-3.5 rounded-xl font-bold hover:bg-opacity-95 hover:shadow-lg hover:shadow-blue-900/10 active:scale-[0.99] transition duration-200 text-sm mt-4 cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
            ...
            <div className="text-center mt-8 text-xs font-medium text-gray-500">
              ...
              <Link to="/register" className="text-[#0D3B66] hover:underline font-bold">
        ...
        {/* SECTION DROITE : DESIGN PROMOTIONNEL APP MOBILE */}
        <div className="lg:col-span-6 bg-[#0D3B66] p-8 md:p-16 flex flex-col justify-between text-white relative overflow-hidden">
          ...
          <div className="absolute -top-24 -right-24 w-72 h-72 rounded-full border border-white/5 pointer-events-none"></div>
          ...
          <div className="my-auto text-center lg:text-left flex flex-col items-center lg:items-start max-w-lg mx-auto">
            ...
            {/* Mockup Smartphone */}
            <div className="relative w-full max-w-[280px] h-[450px] bg-slate-900 rounded-[3rem] border-4 border-slate-700/80 shadow-2xl p-3 flex flex-col overflow-hidden mb-10 group transform hover:scale-105 transition duration-500">
            ...
            {/* Application Stores Links */}
            <div className="flex flex-wrap justify-center lg:justify-start gap-4 w-full">
              <a href="#" className="flex items-center gap-3 bg-white/10 hover:bg-white/15 border border-white/10 rounded-xl px-4 py-2.5 transition active:scale-95 duration-200 text-left w-full sm:w-auto min-w-[145px]">
                <img src={GooglePlayIcon} alt="Google Play" className="w-6 h-6 text-white fill-current" />
                <div>
